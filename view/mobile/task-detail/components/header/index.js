...
export default class Header extends React.Component {
    ...
    showAddTagPrompt = async () => {
        const { addTagHandle } = this.props

        const tagInstance = await Prompt({ title: '请输入新标签', placeholder: '请输入新标签' })
        if (tagInstance.result !== 1) return

        addTagHandle(tagInstance.data)
    }
    ...
    render() {
        ...
        return <>
            <div className='header-top-left flex-rest flex-start'>
                <div className='top-left-item flex-start-center' onClick={() => !isEditDiff && window.history.back()}>
            ...
            <div className='header-top-right flex-start'>
                <div className='top-right-item'
                    onClick={this.showAddTagPrompt}
                >新增标签</div>
                <div className='top-right-item'
                    onClick={selectProgressHandle}
                >{progress || 'not-start'}</div>
            </div>
        </>
    }
}
