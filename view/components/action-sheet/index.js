...
/**
 * 
 * @param {options} object value label
 * @returns 
 */
const ActionSheet = ({ title, options, isMultiple }) => {
    ...
    const selectHandle = selected => {
        resolveHandle(selected)
        destroy()
    }
    const cancelHandle = () => {
        rejectHandle(new Error('cancel'))
        destroy()
    }
    ...
    const SingleActionSheet = () => <>
        ...
        <div style={style.selector}>{options.map(({ value, label }, key) =>
            ...
            >
                <div style={style.descriptionOverHidden}>{label}</div>
            </div>
        )}</div>
    </>
        ...
        render() {
            ...
            return <>
                ...
                <div style={style.selector}>{multipleOptions.map((option, key) =>
                    ...
                    >
                        <div style={style.multipleLable}>
                            <div style={style.descriptionOverHidden}>{option.label}</div>
                        </div>
