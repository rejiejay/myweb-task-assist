...
export class AddTaskProgressPlan extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            title: '',
    ...
    confirmHandle = async () => {
        ...
        const fetchInstance = await service.progress.addProgressPlanByTask(
            taskId,
            {
                title,
                content,
    ...
    render() {
        ...
        return <div className='windows-add-task-plan'>
            <TaskDetail
                title={title}
                content={content}
                specific={specific}
